...
function sendRequestUpdateCart(itemId, quantity) {
    ...
    xmlHttp.open("GET", queryString, true);//true代表异步，不等待servlet返回结果
...
function defaultCheckout(){
    console.log("default");
    location.href = "viewOrderForm?confirm=default";
}

function continueCheckout(){
    if(confirm("You have an unfinished Order. Do you want to continue?")){
        console.log("continue");
        location.href = "viewOrderForm?confirm=continue";
    }else{
        defaultCheckout()
    }
}
...
$(function () {
    ...
    function removeCartItem() {
        ...
        $.get("removeItemFromCart?workingItemId=" + str);
    ...
    $("#new-cartItem-box").typeahead({
        ...
        afterSelect: function (item) {//选中一条数据后的回调函数,jquery和typeahead会各触发一次
            if(flag){
                ...
                var param = "addItemByName?productName=" + item;
                setTimeout(function () {
                    $.get(param, function () {
                        window.location.href = "viewCart";
